...
import com.doctoraak.doctoraakpatient.adapters.RadiologyOrdersAdapter
...
import com.doctoraak.doctoraakpatient.model.userOrdersModels.RadiologyOrder
...
class RadiologyOrdersFragment : Fragment()
{
    private lateinit var adapter: RadiologyOrdersAdapter
    ...
    companion object {
        @JvmStatic
        fun newInstance() = RadiologyOrdersFragment()
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View?
    {
        ...
        with(binding.rvMyOrders)
        {
            ...
            this@RadiologyOrdersFragment.adapter = RadiologyOrdersAdapter(arrayListOf(), context)
            adapter = this@RadiologyOrdersFragment.adapter
        }
        ...
        adapter.setOnItemCancelOrderListener(object : RadiologyOrdersAdapter.ClickCancelOrder {
            override fun onClick(model: RadiologyOrder, position: Int, aView: View) {
                orderPosition = position
                cancelOrderDialog(model)
            }
        })

        adapter.setOnItemViewPhotoListener(object : RadiologyOrdersAdapter.ClickViewPhoto {
            override fun onClick(url: String) {
                val dialog = ViewPhotoFragment.newInstance(url , OrderType.RADIOLOGY.toString())
    ...
    val getAllOrders = { viewModel.getMyOrdersRadiology(Utils.getUserId(), Utils.getUser().apiToken
        , {if (it) {
    ...
    override fun onResume() {
        super.onResume()
        if (viewModel.radiologyReservation.value == null) {
            getAllOrders()
        } else {
            adapter.setData(viewModel.radiologyReservation.value!!.data)
        }
    }


    private fun cancelOrderDialog(model: RadiologyOrder) {
    ...
    private fun observeData()
    {
        ...
        viewModel.radiologyReservation.observe(this, Observer {
            if (it.data.size > 0) {
