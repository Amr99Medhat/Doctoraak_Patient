...
import kotlinx.android.synthetic.main.icu_item.view.*
import java.util.*

class IcusAdapter(val context: Context) :
    PagingAdapter<Icu>() {
    ...
    inner class  MyViewHolder(view: View) : RecyclerView.ViewHolder(view)
        , View.OnClickListener, PagingHolder<Icu> {
        override fun onClick(v: View?) {
            mClickListener.onClick(list[adapterPosition] , v!!)
        }
        val icuName = view.tv_name
        ...
        override fun bind(item: Icu) {
            icuName.text = Utils.getTextForAppLanguage(item.name,item.name_ar,item.name_fr)
            ...
            image.text = Utils.getTextForAppLanguage(item.name,item.name_ar,item.name_fr)
                .substring(0 , 1).toUpperCase(Locale.US)
            image.setBackgroundResource(colors[(0..3).random()])
            try { rate.rating = (item.rate).toFloat() } catch (e: Exception) { }
