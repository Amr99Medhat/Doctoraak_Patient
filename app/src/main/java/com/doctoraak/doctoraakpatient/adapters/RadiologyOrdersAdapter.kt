...
class RadiologyOrdersAdapter(val order: ArrayList<RadiologyOrder>, val context: Context) :
    RecyclerView.Adapter<RadiologyOrdersAdapter.MyViewHolder>() 
{
    ...
    interface ClickCancelOrder {
        fun onClick(model: RadiologyOrder, position: Int, aView: View)
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        return MyViewHolder(
            LayoutInflater.from(parent.context)
                .inflate(R.layout.radiology_order_item, parent, false)
        )
    }
    ...
    fun setData(data: ArrayList<RadiologyOrder>) {
        clearData()
        order.addAll(data)
    ...
    private fun InializeUI(holder: MyViewHolder, reservation: RadiologyOrder)
    {
        ...
        else
        {
            ...
            }).error(R.drawable.ic_radiology_order)
                .placeholder(R.drawable.ic_radiology_order).into(holder.image)
            View.VISIBLE
        }
        ...
        with(holder.rv_medicines)
        {
            ...
            if (reservation.details != null) {
                adapter = RadiologyMyOrderAdapter(ArrayList(reservation.details), context, Utils.getRays().data)
            }
