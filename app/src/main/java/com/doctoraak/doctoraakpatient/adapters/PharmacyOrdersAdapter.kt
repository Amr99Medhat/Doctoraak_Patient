...
class PharmacyOrdersAdapter(val order: ArrayList<PharmacyOrders>, val context: Context) :
    RecyclerView.Adapter<PharmacyOrdersAdapter.MyViewHolder>()
{
    ...
    interface ClickCancelOrder {
        fun onClick(model: PharmacyOrders, position: Int, aView: View)
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        return MyViewHolder(
            LayoutInflater.from(parent.context)
                .inflate(R.layout.pharmacy_order_item, parent, false)
        )
    }
    ...
    fun setData(data: ArrayList<PharmacyOrders>) {
        clearData()
        order.addAll(data)
    ...
    private fun InializeUI(holder: MyViewHolder, reservation: PharmacyOrders)
    {
        ...
        {
            Glide.with(context).load(reservation.photo).listener(object : RequestListener<Drawable?> {
            ...
            }).error(R.drawable.ic_pharmacy_order)
                .placeholder(R.drawable.ic_pharmacy_order).into(holder.image)
            VISIBLE
        }
        ...
        with(holder.rv_medicines)
        {
            ...
            if (reservation.details != null) {
                adapter = MedicinesMyOrderAdapter(reservation.details, context, Utils.getMedicines().data
                    , Utils.getMedicinesType().data)
            }
